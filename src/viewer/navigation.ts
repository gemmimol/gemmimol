...
export class Navigator {
  ...
  prev_site(): SiteNavItem | null {
    if (this.sites.length === 0) return null;
    this.current_site = this.current_site <= 0 ?
      this.sites.length - 1 : this.current_site - 1;
    return this.sites[this.current_site];
  }
  ...
  parse_cid(cid: string): { chain?: string; resno?: number; atom?: string } | null {
    // Simple CID parser: "/A/12/CA" -> {chain: "A", resno: 12, atom: "CA"}
    // "/A/*/*" -> {chain: "A"}
    ...
    if (parts[1] && parts[1] !== '*') {
      result.resno = parseInt(parts[1], 10);
    }
    if (parts[2] && parts[2] !== '*') {
      result.atom = parts[2];
    }
  ...
  find_atom_by_cid(cid: string): { bag: ModelBag; atom: any } | null {
    ...
    for (const bag of this.model_bags) {
      for (const atom of bag.model.atoms) {
        ...
        if (parsed.resno !== undefined && atom.seqid !== parsed.resno) continue;
  ...
  // Find blobs (unmodeled density) - placeholder for future implementation
  find_blobs(): number[][] {
    // Would analyze map data to find unmodeled density peaks
